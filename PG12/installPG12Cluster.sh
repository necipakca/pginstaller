...
function helpFunction(){
   ...
   echo -e "\t-e optinal- DCS root directory path. e.g. PG12_PROD_CLS.if you not provide default value is  PG_+ Scope Name"
...
install_ssh_id(){
    ...
    sshpass -p "$OS_ROOT_PASSWORD" ssh-copy-id -o StrictHostKeyChecking=no root@$1 &>> installPG12Cluster.log
...
install_ssh_id_to_all_servers(){
    ...
    do
        ...
        sudo -u ${1} sshpass -p "$2" ssh-copy-id -o StrictHostKeyChecking=no ${1}@${SERVER_IP} &>> installPG12Cluster.log
...
get_disk_size(){
    echo $(ssh -q -oStrictHostKeyChecking=no root@$1  DISK_PATH=$2 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
checkInternetConnections(){
    checkCommandStatus $(ssh -q -oStrictHostKeyChecking=no root@$1 IP=${1}  2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
ENDSSH
    ...
    checkCommandStatus $(ssh -q -oStrictHostKeyChecking=no root@$1 IP=${1}  2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
        curl -s --head https://yum.postgresql.org/12/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm | head -n 1 | grep "HTTP/1.[01] [23].." > /dev/null
        echo "${IP}:    Checking server can access the url https://yum.postgresql.org/12/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm for PG12 repo install"
ENDSSH
    )
}

checkOSUsers(){
    checkCommandStatus $(ssh -q -oStrictHostKeyChecking=no root@$1 IP=${1} PG_SUPER_USER_PASSWORD=${PG_SUPER_USER_PASSWORD} 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
ENDSSH
    ...
    checkCommandStatus $(ssh -q -oStrictHostKeyChecking=no root@$1 IP=${1} PG_BACKREST_USER_PASSWORD=${PG_BACKREST_USER_PASSWORD} 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
check_disk_mount_status(){
    local STATUS_DATA_MOUNT=$(ssh -q -oStrictHostKeyChecking=no root@$1  DISK_PATH=$2 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
validate_env() {
  ...
  for IP in "${LIST[@]}"; do
    ...
    rm -rf  installPG12Cluster.log
    rsync -qaz ./installPG12Cluster root@"${IP}":/root/
...
FileSystemUtil(){
    ...
    createDiskAndMount(){
        ...
        wipefs -aqf "${DISK_PATH}" &>> installPG12Cluster.log
        checkCommandStatus  "       wipefs -aqf "${DISK_PATH}""
        parted -s ${DISK_PATH} mklabel gpt &>> installPG12Cluster.log;
        checkCommandStatus "        mklabel gpt"
        parted -s ${DISK_PATH} "mkpart primary 1 -1" &>> installPG12Cluster.log;
        checkCommandStatus "        mkpart primary 1"
        parted -s ${DISK_PATH} set 1 lvm on &>> installPG12Cluster.log;
        checkCommandStatus "        set 1 lvm on"
        pvcreate "${DISK_PATH}1" &>> installPG12Cluster.log
        checkCommandStatus "        pvcreate ${DISK_PATH}1"
        vgcreate --force --yes vg_"${SCOPE_NAME}"_"${LVM_ALIAS}" "${DISK_PATH}1" &>> installPG12Cluster.log
        checkCommandStatus  "       vgcreate"
        lvcreate --yes --name lv_"${SCOPE_NAME}"_"${LVM_ALIAS}" -l 100%FREE vg_"${SCOPE_NAME}"_"${LVM_ALIAS}" &>> installPG12Cluster.log
        checkCommandStatus  "       lvcreate"
        mkfs.ext4 -F /dev/vg_"${SCOPE_NAME}"_"${LVM_ALIAS}"/lv_"${SCOPE_NAME}"_"${LVM_ALIAS}" &>> installPG12Cluster.log
        checkCommandStatus  "       mkfs.ext4"
        mkdir -p /pg_${SCOPE_NAME}/mounts/${LVM_ALIAS} &>> installPG12Cluster.log
        ...
        mount /pg_${SCOPE_NAME}/mounts/${LVM_ALIAS}  &>> installPG12Cluster.log
        ...
        # test disk
        dd if=/dev/zero of=/pg_${SCOPE_NAME}/mounts/${LVM_ALIAS}/file.txt count=10 bs=1024 &>> installPG12Cluster.log
    ...
    if [[ $(partprobe -d -s ${DISK_PATH} | grep 'partitions') = "" ]]; then
        ...
        if [[ ${lv_name} ]] ; then
            #lvremove --force --yes ${vg_name} &>> installPG12Cluster.log ;
            #checkCommandStatus  "      lvremove --force --yes ${vg_name}"
            vgremove --force --yes ${vg_name} &>> installPG12Cluster.log ;
...
CentOsPacksInstallerAndKernel(){
    ...
    # TODO  NOT WORK check later   yum install http://download.postgresql.org/pub/repos/yum/12/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm
    wget -q --output-document="./pgdg-redhat-repo-latest.noarch.rpm" http://download.postgresql.org/pub/repos/yum/12/redhat/rhel-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm
    checkCommandStatus "PG12 Repo Download RPM"
    sudo yum install ./pgdg-redhat-repo-latest.noarch.rpm -y >&-
    rm ./pgdg-redhat-repo-latest.noarch.rpm


    # Postgres install to server
    sudo yum groupinstall "PostgreSQL Database Server 12 PGDG" -y >&-
    checkCommandStatus "Installing PostgreSQL Database Server 12 PGDG"
    # Postgres tools install
    sudo yum install postgresql12-plpython pg_activity pgbadger pg_stat_kcache12 pgaudit13_12 pg_qualstats12 pg_squeeze12 timescaledb_12 cstore_fdw_12 citus_12 pgbackrest -y  >&-
...
EtcdInstaller(){
    ...
    check_etcd_status(){
        ssh -q -oStrictHostKeyChecking=no root@$1 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
ENDSSH
    ...
    adjust_firewall_etcd(){
          ...
          do
                ...
                ssh -q -oStrictHostKeyChecking=no root@${SERVER_IP} FIREWALL_SERVER_IP=${FIREWALL_SERVER_IP} 2>> installPG12Cluster.log SERVER_IP=${SERVER_IP} 'bash -s' <<-'ENDSSH'
...
ENDSSH
    ...
    start_setup_etcd() {
      ...
      do
       ...
       ssh -oStrictHostKeyChecking=no root@"${SERVER_IP}"  NAME=${NAME} SERVER_IP=${SERVER_IP} SCOPE_NAME=${SCOPE_NAME} ETCD_INITIAL_CLUSTER=${ETCD_INITIAL_CLUSTER}  2>> installPG12Cluster.log 'bash -s' <<-'ENDSSH'
            check_program_exist()
            {
              command -v "$1" >/dev/null 2>&1
            }
...
ENDSSH
      ...
      do
       ...
       ssh -oStrictHostKeyChecking=no root@"${SERVER_IP}" SERVER_IP=${SERVER_IP} 2>> installPG12Cluster.log 'bash -s' <<-'ENDSSH'
            checkCommandStatus(){
                if [[ $? -eq 0 ]]; then
                    echo "${SERVER_IP} :$1..etcd_install....."${GREEN}"OK${NORMAL}";
                else
...
create_master_template(){
    ...
    ssh -oStrictHostKeyChecking=no root@${1} SERVER_IP=${1} SCOPE_NAME=${2} PG_PORT=${PG_PORT}  ETCD_PASSWORD=${ETCD_PASSWORD} 2>> installPG12Cluster.log 'bash -s' <<-'ENDSSH'
...
#  post_init: /pg01/mounts/data_01/patroni/postgresql-12-pg01-5432-patroni-{{ project_name }}-postInit.sh
...
#    - custom_bb
#  custom_bb:
#    command : /pg01/mounts/data_01/patroni/postgresql-12-pg01-5432-patroni-{{ project_name }}-baseBackup.sh
...
InstallPatroni(){
        ...
        check_patroni_status(){
            echo $(
            ssh -q -oStrictHostKeyChecking=no root@$1 SCOPE_NAME=${SCOPE_NAME} 2>> installPG12Cluster.log  'bash -s' <<-'ENDSSH'
...
ENDSSH
        ...
        adjust_firewallPatroni(){
          ...
          do
                FIREWALL_SERVER_IP=${LIST[$i]}
                ssh -q -oStrictHostKeyChecking=no root@${SERVER_IP} FIREWALL_SERVER_IP=${FIREWALL_SERVER_IP} 2>> installPG12Cluster.log SERVER_IP=${SERVER_IP} PG_PORT=${PG_PORT} 'bash -s' <<-'ENDSSH'
...
ENDSSH
        ...
        start_setup_patroni() {
          ...
          do
           ...
           ssh -oStrictHostKeyChecking=no root@"${SERVER_IP}"  NAME=${NAME} SERVER_IP=${SERVER_IP} SCOPE_NAME=${SCOPE_NAME} NAMESPACE=${DSC_ROOT_PATH} ETCD_INITIAL_CLUSTER="${ETCD_INITIAL_CLUSTER}" WAL_PATH="/pg_${SCOPE_NAME}/mounts/wal_m/wal" DATA_PATH="/pg_${SCOPE_NAME}/mounts/data_m/data" PG_BIN_DIR="/usr/pgsql-12/bin" PG_PORT=${PG_PORT} ETCD_PASSWORD=${ETCD_PASSWORD} PG_SUPER_USER_PASSWORD=${PG_SUPER_USER_PASSWORD} REPLICATION_USER_PASSWORD=${REPLICATION_USER_PASSWORD} 'bash -s' <<-'ENDSSH'
...
cent_os_env_installer() {
  ...
  for IP in "${LIST[@]}"; do
    ...
    ssh -oStrictHostKeyChecking=no root@"${IP}" IP=${IP} IP_LIST_OF_CLUSTER=${IP_LIST_OF_CLUSTER} SCOPE_NAME=${SCOPE_NAME} DATA_PATH=${DATA_PATH} WAL_PATH=${WAL_PATH} DSC_ROOT_PATH=${DSC_ROOT_PATH} PG_PORT=${PG_PORT} ETCD_PASSWORD=${ETCD_PASSWORD} REPLICATION_USER_PASSWORD=${REPLICATION_USER_PASSWORD} PG_SUPER_USER_PASSWORD=${PG_SUPER_USER_PASSWORD} PG_BACKREST_USER_PASSWORD=${PG_BACKREST_USER_PASSWORD} 2>> installPG12Cluster.log 'bash -s' <<-'ENDSSH'
    /root/installPG12Cluster -n "NOT_NEED" -p ${IP_LIST_OF_CLUSTER} -s "${SCOPE_NAME}" -d "${DATA_PATH}" -w "${WAL_PATH}" -e "${DSC_ROOT_PATH}" -k "${PG_PORT}" -g ${ETCD_PASSWORD} -t ${REPLICATION_USER_PASSWORD} -y ${PG_SUPER_USER_PASSWORD} -u ${PG_BACKREST_USER_PASSWORD} -x 1
    if [[ $? -eq 0 ]]; then
...
ENDSSH
  ...
  /root/installPG12Cluster -n "NOT_NEED" -p ${IP_LIST_OF_CLUSTER} -s "${SCOPE_NAME}" -d "${DATA_PATH}" -w "${WAL_PATH}" -e "${DSC_ROOT_PATH}" -k "${PG_PORT}" -g ${ETCD_PASSWORD}  -t ${REPLICATION_USER_PASSWORD} -y ${PG_SUPER_USER_PASSWORD} -u ${PG_BACKREST_USER_PASSWORD} -x 2
  checkCommandStatus
  /root/installPG12Cluster -n "NOT_NEED" -p ${IP_LIST_OF_CLUSTER} -s "${SCOPE_NAME}" -d "${DATA_PATH}" -w "${WAL_PATH}" -e "${DSC_ROOT_PATH}" -k "${PG_PORT}" -g ${ETCD_PASSWORD}  -t ${REPLICATION_USER_PASSWORD} -y ${PG_SUPER_USER_PASSWORD} -u ${PG_BACKREST_USER_PASSWORD} -x 3
  checkCommandStatus
...
if [[ -f "/etc/centos-release" ]]; then
    ...
    if [[ -z ${RUN_LEVEL} ]] ;then
        bannerWithoutTime "PosgreSQL  12  HA  Cluster  Installer"
